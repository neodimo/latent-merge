...
2. [Gonzo] Unblock ONE real relight backend on the 3080 Ti: fix IC-Light v2 weights/contract (correct package + `config.json`) OR stand up IC-Light/FLUX relight via local diffusers. Goal: first actual relight output to compare against PCT-Net. 2026-06-20 through 2026-06-22 preflights narrowed this: repo symlinked SD1.5 IC-Light weights are present and channel-compatible (`fbc` 12-channel, `fc` 8-channel), `libcuda`/NVML libraries resolve, but this cron scope cannot access NVIDIA hardware/driver state (`/dev/nvidia*` absent, `/proc/driver/nvidia/version` absent, `torch.cuda.is_available=false`, `nvidia-smi` cannot communicate with driver). The 2026-06-22 PCI inventory shows only AMD Strix Halo display hardware and no NVIDIA PCI device in this environment. Evidence: `reports/ic-light-runtime-check-20260620.json`, `reports/ic-light-runtime-check-20260621-morning.json`, `reports/ic-light-runtime-check-20260621-afternoon.json`, `reports/ic-light-runtime-check-20260622-morning.json`; reusable check: `PYTHONPATH=. python3 scripts/check_ic_light_runtime.py --require-cuda`.
